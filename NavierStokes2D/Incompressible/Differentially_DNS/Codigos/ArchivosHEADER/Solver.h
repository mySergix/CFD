...
class CalculoSolver{
	...
	//Valores iniciales de los mapas de propiedades
		...
		//Estudio turbulencia en el Differential Heated
		double TiempoCoeficientes;
		double SumaNUmean;
		double SumaNUmax;

		double SumaPosNUmaxMean;

		double SumaUXmaxmean;
		double SumaPOSuxmean;
		double SumaVXmaxmean;
		double SumaPOSvxmean;

		double SumaUYmaxmean;
		double SumaPOSuymean;
		
		double SumaVYmaxmean;
		double SumaPOSvymean;
		
		double SumaTmaxmean;
		double SumaPOStmaxmean;
...
//----------------------------------------------------------------------------------------------------------------------------------
	...
	public:
		...
		void CalculoF(Mallador);
		void CalculoFinicial(Mallador);
		...
		void AverageFlowTurbulence(Mallador, double, double);

		void CalculoStrouhal(double, double);
		void PrintVTK(Mallador, int);
		void PrintGnuPlot(int , double, Mallador);
		void VelocidadesVTK(double**, double**, Mallador, string, string);
		void EscalarVTK(double**, Mallador, string, string);

		//Calculo del mapa de temperaturas
		double BoussinesqU(Mallador, int, int);
		double BoussinesqV(Mallador, int, int);
		...
		void ResultadosNusselt(Mallador, double);
