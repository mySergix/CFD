	...
	if(Problema == 1 || Problema == 2){
		...
		if(Progresion == 1){
		 ...
		 double	ny3 = NY3;
			...
			//Análisis Square cylinder malla tanh
			/*
			NY2 = 100;
			NY1 = 121;
			NY3 = 121;
...
int Mallador::Get_OpcX(){ return OpcionX;}	
int Mallador::Get_OpcY(){ return OpcionY;}
				...
				for (j = 0; j < NYP+1; j++){
					...
					if(j == 0){
						...
						if(Opcion2 == 1){//Regular
							...
							J = j;
							M2[i+NXA][j+NYA][1] = Y1 + Y*((tanh(StretFactor2*((J-ny)/ny)) + tanh(StretFactor2))/tanh(StretFactor2));
							//M2[i+NXA][j+NYA][1] = Y1 + Y*(1 - cos(((J/ny)*(PI/2))));
						}
						else if(Opcion2 == 4){//Senoidal
							J = j;
							M2[i+NXA][j+NYA][1] = Y1 + Y*((tanh(StretFactor2*(J/ny)) + tanh(StretFactor2))/tanh(StretFactor2) - 1);
			...
			for (i = 0; i < NXP+1; i++){
				for (j = 0; j < NYP; j++){
						if(Opcion1 == 1){//Regular
							...
							I = i;
							M3[i+NXA][j+NYA][0] = X1 + X*((tanh(StretFactor*((I-nx)/nx)) + tanh(StretFactor))/tanh(StretFactor));
							//M3[i+NXA][j+NYA][0] = X1 + X*(1 - cos((I/nx)*(PI/2)));
						}
						else if(Opcion1 == 4){//Senoidal 
							I = i;
							M3[i+NXA][j+NYA][0] = X1 + X*((tanh(StretFactor*(I/nx)) + tanh(StretFactor))/tanh(StretFactor) - 1);
...
//Superficies de las caras de los volúmenes de control
...
void Mallador::PrintTxt(){
	...
	FILE *fp1;
	fp1 = fopen("/home/sergiogus/Desktop/CTTC/Turbulencia/Resultados/Malla/M1.txt","w");
	for(i = 0; i < NXTOT; i++){
        	for(j = 0; j < NYTOT; j++){
			fprintf(fp1,"%f %f \n",M1[i][j][0],M1[i][j][1]);
		}
	...
	FILE *fp2;
	fp2 = fopen("/home/sergiogus/Desktop/CTTC/Turbulencia/Resultados/Malla/M2.txt","w");
	for(i = 0; i < NXTOT; i++){
        	for(j = 0; j < NYTOT+1; j++){
			fprintf(fp1,"%f %f \n",M2[i][j][0],M2[i][j][1]);
		}
	...
	FILE *fp3;
	fp3 = fopen("/home/sergiogus/Desktop/CTTC/Turbulencia/Resultados/Malla/M3.txt","w");
	for(i = 0; i < NXTOT+1; i++){
        	for(j = 0; j < NYTOT; j++){
			fprintf(fp1,"%f %f \n",M3[i][j][0],M3[i][j][1]);
		}
