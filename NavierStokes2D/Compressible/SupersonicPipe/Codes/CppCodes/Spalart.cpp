...
int i, j;
	...
	for(i = 0; i < NA; i++){
		for(j = 0; j < NRad; j++){
			SA_Termino2[i][j] = Cb1*Smodel[i][j]*vpresent[i][j]*RhoPres[i][j];
		}
	}
}
...
//Cálculo del término 6 de la ecuación de Spalart-Allmaras
void Solver::SA_Term6(Mesher MESH){
int i, j;

	//Centro 
	for(i = 1; i < NA-1; i++){
		for(j = 1; j < NRad-1; j++){
			SA_Termino6[i][j] = (1.0/(MESH.VolMP[i][j]*Sigma))*(muBase[i][j]/RhoPres[i][j] + vpresent[i][j])*(
								((0.50*(RhoPres[i][j] + RhoPres[i+1][j]) - 0.50*(RhoPres[i][j] + RhoPres[i-1][j]))/MESH.DeltasMP[i][j][0])*((0.50*(vpresent[i][j] + vpresent[i+1][j]) - 0.50*(vpresent[i][j] + vpresent[i-1][j]))/MESH.DeltasMP[i][j][0])
							  + ((0.50*(RhoPres[i][j] + RhoPres[i][j+1]) - 0.50*(RhoPres[i][j] + RhoPres[i][j-1]))/MESH.DeltasMP[i][j][1])*((0.50*(vpresent[i][j] + vpresent[i][j+1]) - 0.50*(vpresent[i][j] + vpresent[i][j-1]))/MESH.DeltasMP[i][j][1])	
							  );
		}
	}
	...
	for(j = 1; j < NRad-1; j++){
		//Parte izquierda
		SA_Termino6[0][j] = (1.0/(MESH.VolMP[0][j]*Sigma))*(muBase[0][j]/RhoPres[0][j] + vpresent[0][j])*(
								((0.50*(RhoPres[0][j] + RhoPres[1][j]) - RhoLeft[j])/MESH.DeltasMP[0][j][0])*((0.50*(vpresent[0][j] + vpresent[1][j]) - vleft[j])/MESH.DeltasMP[0][j][0])
							  + ((0.50*(RhoPres[0][j] + RhoPres[0][j+1]) - 0.50*(RhoPres[0][j] + RhoPres[0][j-1]))/MESH.DeltasMP[0][j][1])*((0.50*(vpresent[0][j] + vpresent[0][j+1]) - 0.50*(vpresent[0][j] + vpresent[0][j-1]))/MESH.DeltasMP[0][j][1])	
							  );
		//Parte derecha
		SA_Termino6[NA-1][j] = (1.0/(MESH.VolMP[NA-1][j]*Sigma))*(muBase[NA-1][j]/RhoPres[NA-1][j] + vpresent[NA-1][j])*(
								((RhoRight[j] - 0.50*(RhoPres[NA-1][j] + RhoPres[NA-2][j]))/MESH.DeltasMP[NA-1][j][0])*((vright[j] - 0.50*(vpresent[NA-1][j] + vpresent[NA-2][j]))/MESH.DeltasMP[NA-1][j][0])
							  + ((0.50*(RhoPres[NA-1][j] + RhoPres[NA-1][j+1]) - 0.50*(RhoPres[NA-1][j] + RhoPres[NA-1][j-1]))/MESH.DeltasMP[NA-1][j][1])*((0.50*(vpresent[NA-1][j] + vpresent[NA-1][j+1]) - 0.50*(vpresent[NA-1][j] + vpresent[NA-1][j-1]))/MESH.DeltasMP[NA-1][j][1])	
							  );
	...
	for(i = 1; i < NA-1; i++){
		//Parte abajo
		SA_Termino6[i][0] = (1.0/(MESH.VolMP[i][0]*Sigma))*(muBase[i][0]/RhoPres[i][0] + vpresent[i][0])*(
								((0.50*(RhoPres[i][0] + RhoPres[i+1][0]) - 0.50*(RhoPres[i][0] + RhoPres[i-1][0]))/MESH.DeltasMP[i][0][0])*((0.50*(vpresent[i][0] + vpresent[i+1][0]) - 0.50*(vpresent[i][0] + vpresent[i-1][0]))/MESH.DeltasMP[i][0][0])
							  + ((0.50*(RhoPres[i][0] + RhoPres[i][1]) - RhoDown[i])/MESH.DeltasMP[i][0][1])*((0.50*(vpresent[i][0] + vpresent[i][1]) - vdown[i])/MESH.DeltasMP[i][0][1])	
							  );

		//Parte arriba
		SA_Termino6[i][NRad-1] = (1.0/(MESH.VolMP[i][NRad-1]*Sigma))*(muBase[i][NRad-1]/RhoPres[i][NRad-1] + vpresent[i][NRad-1])*(
								((0.50*(RhoPres[i][NRad-1] + RhoPres[i+1][NRad-1]) - 0.50*(RhoPres[i][NRad-1] + RhoPres[i-1][NRad-1]))/MESH.DeltasMP[i][NRad-1][0])*((0.50*(vpresent[i][NRad-1] + vpresent[i+1][NRad-1]) - 0.50*(vpresent[i][NRad-1] + vpresent[i-1][NRad-1]))/MESH.DeltasMP[i][NRad-1][0])
							  + ((RhoUp[i] - 0.50*(RhoPres[i][NRad-1] + RhoPres[i][NRad-2]))/MESH.DeltasMP[i][NRad-1][1])*((vup[i] - 0.50*(vpresent[i][NRad-1] + vpresent[i][NRad-2]))/MESH.DeltasMP[i][NRad-1][1])	
							  );

	}

	//Esquina abajo izquierda
	SA_Termino6[0][0] = (1.0/(MESH.VolMP[0][0]*Sigma))*(muBase[0][0]/RhoPres[0][0] + vpresent[0][0])*(
								((0.50*(RhoPres[0][0] + RhoPres[1][0]) - RhoLeft[0])/MESH.DeltasMP[0][0][0])*((0.50*(vpresent[0][0] + vpresent[1][0]) - vleft[0])/MESH.DeltasMP[0][0][0])
							  + ((0.50*(RhoPres[0][0] + RhoPres[0][1]) - RhoDown[0])/MESH.DeltasMP[0][0][1])*((0.50*(vpresent[0][0] + vpresent[0][1]) - vdown[0])/MESH.DeltasMP[0][0][1])	
							  );

	//Esquina arriba izquierda
	SA_Termino6[0][NRad-1] = (1.0/(MESH.VolMP[0][NRad-1]*Sigma))*(muBase[0][NRad-1]/RhoPres[0][NRad-1] + vpresent[0][NRad-1])*(
								((0.50*(RhoPres[0][NRad-1] + RhoPres[1][NRad-1]) - RhoLeft[NRad-1])/MESH.DeltasMP[0][NRad-1][0])*((0.50*(vpresent[0][NRad-1] + vpresent[1][NRad-1]) - vleft[NRad-1])/MESH.DeltasMP[0][NRad-1][0])
							  + ((RhoUp[0] - 0.50*(RhoPres[0][NRad-1] + RhoPres[0][NRad-2]))/MESH.DeltasMP[0][NRad-1][1])*((vup[0] - 0.50*(vpresent[0][NRad-1] + vpresent[0][NRad-2]))/MESH.DeltasMP[0][NRad-1][1])	
							  );

	//Esquina abajo derecha
	SA_Termino6[NA-1][0] = (1.0/(MESH.VolMP[NA-1][0]*Sigma))*(muBase[NA-1][0]/RhoPres[NA-1][0] + vpresent[NA-1][0])*(
								((RhoRight[0] - 0.50*(RhoPres[NA-1][0] + RhoPres[NA-2][0]))/MESH.DeltasMP[NA-1][0][0])*((vright[0] - 0.50*(vpresent[NA-1][0] + vpresent[NA-2][0]))/MESH.DeltasMP[NA-1][0][0])
							  + ((0.50*(RhoPres[NA-1][0] + RhoPres[NA-1][1]) - RhoDown[NA-1])/MESH.DeltasMP[NA-1][0][1])*((0.50*(vpresent[NA-1][0] + vpresent[NA-1][1]) - vdown[NA-1])/MESH.DeltasMP[NA-1][0][1])	
							  );

	//Esquina arriba derecha
	SA_Termino6[NA-1][NRad-1] = (1.0/(MESH.VolMP[NA-1][NRad-1]*Sigma))*(muBase[NA-1][NRad-1]/RhoPres[NA-1][NRad-1] + vpresent[NA-1][NRad-1])*(
								((RhoRight[NRad-1] - 0.50*(RhoPres[NA-1][NRad-1] + RhoPres[NA-2][NRad-1]))/MESH.DeltasMP[NA-1][NRad-1][0])*((vright[NRad-1] - 0.50*(vpresent[NA-1][NRad-1] + vpresent[NA-2][NRad-1]))/MESH.DeltasMP[NA-1][NRad-1][0])
							  + ((RhoUp[NA-1] - 0.50*(RhoPres[NA-1][NRad-1] + RhoPres[NA-1][NRad-2]))/MESH.DeltasMP[NA-1][NRad-1][1])*((vup[NA-1] - 0.50*(vpresent[NA-1][NRad-1] + vpresent[NA-1][NRad-2]))/MESH.DeltasMP[NA-1][NRad-1][1])	
							  );
