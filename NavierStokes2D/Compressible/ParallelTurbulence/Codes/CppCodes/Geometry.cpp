...
Geometry::Geometry(Memory M1, ReadData R1){
	...
	//Datos geometría sección divergente
	CoeficientesExpresion = M1.AllocateDouble(4,1,1); //Coeficientes de la expresión matemática
	CoeficientesExpresionSup = M1.AllocateDouble(4,1,1); //Coeficientes de la expresión matemática
...
double I, J, K;
	...
	XEpositions = M1.AllocateDouble(AxialCoordIterations+1,1,1);
	UpperRE = M1.AllocateDouble(AxialCoordIterations+1,1,1);
	LowerRE = M1.AllocateDouble(AxialCoordIterations+1,1,1);
...
int Numero;
	TE = ThetaE;
	FILE *fp1;
	fp1 = fopen("/home_nobck/sergiogus/TFG/NumericalResults/Geometry/LimiteCoordenadas.txt","w");
