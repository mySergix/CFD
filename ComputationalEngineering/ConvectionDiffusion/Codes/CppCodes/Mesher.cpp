...
Mesher::Mesher(Memory M1, ReadData R1, ParPro MPI1, int i){
	...
	//Datos sobre la geometría del problema
		...
		NX = R1.NumericalData[0]; //Número de nodos dirección X
		NY = R1.NumericalData[1]; //Número de nodos dirección Y

		//NX = R1.MeshStudyNX[i]; //Número de nodos dirección X
		//NY = R1.MeshStudyNY[i]; //Número de nodos dirección Y
...
double J;
	...
	if(OpcionR == 1){ //Regular
		...
		for(i = 0; i < NX; i++){
			for(j = 0; j < NY + 1; j++){
				J = j;
				MR[GR(i,j,1)] = (0.50*ChannelHeight)*(1.0 + (tanh(StretFactorY*((2.0*J - ny)/ny)) + tanh(StretFactorY))/tanh(StretFactorY) - 1.0);
			}
		}
	}
...
//Cálculo de los volúmenes de control
...
int i,j;
	...
	Get_Volumes(); //Cálculo de los volúmenes de control
